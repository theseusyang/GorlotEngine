...
class MoveTool extends THREE.Object3D {
	constructor() {
		...
		mesh.updateMatrix()
		this.x.add(mesh);
		mesh = new THREE.Mesh(cone_geometry, this.material_red)
		mesh.matrixAutoUpdate = false
		...
		mesh.updateMatrix()
		this.y.add(mesh)
		mesh = new THREE.Mesh(cone_geometry, this.material_green)
		mesh.matrixAutoUpdate = false
		mesh.position.set(0, 1, 0)
		mesh.updateMatrix()
		this.y.add(mesh)
		...
		mesh.updateMatrix()
		this.z.add(mesh)
		mesh = new THREE.Mesh(cone_geometry, this.material_blue)
		mesh.position.set(0, 1, 0)
		mesh.updateMatrix()
		this.z.add(mesh)
		mesh = new THREE.Mesh(cylinder_geometry_big, material_invisible)
	...
	update() {
		if (this.obj !== null) {
			...
			if (this.selected) {
				...
				if (this.selected_x) {
					this.obj.position.x -= Mouse.delta.x * speed * Math.sin(Editor.camera_rotation.x) / scale.x
